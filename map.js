...
$('#Declaration_of_at_risk').click(function(){
  if ($('#Declaration_of_at_risk').is(':checked')){
    for (let i = 0; i < title.length; i++) {
      legends[i].remove();
  }
    legends[6].addTo(map);
    map.addLayer(Declaration_of_at_risk);
  }
  else {
    map.removeLayer(Declaration_of_at_risk);
    legends[6].remove();
  }
});
...
$('#Marine_habitats_2018').click(function(){
  if ($('#Marine_habitats_2018').is(':checked')){
    map.addLayer(Marine_habitats_2018);
    for (let i = 0; i < title.length; i++) {
      legends[i].remove();
  }
    ...
    map.removeLayer(Marine_habitats_2018);
    legends[16].remove();
  }
});
...
/*$('#DEM_AMSS').click(function(){
  if ($('#DEM_AMSS').is(':checked')){
    map.addLayer(DEM_AMSS);
    for (let i = 0; i < title.length; i++) {
      legends[i].remove();
  }
